...
def policy_evaluation(policy, value_function, env, args):
    for i in range(args.iterations):
        updated_value_function = [0] * env.states
        for s in range(env.states):
            updated_value_function[s] = \
                sum([p * (r + args.gamma * value_function[s_next]) for [p, r, s_next] in env.step(s, policy[s])])
...
def policy_improvement(policy, value_function, env, args):
    stable = True
    for s in range(env.states):
        old_action = policy[s]
        policy[s] = \
            argmax([sum([p * (r + args.gamma * value_function[s_next]) for [p, r, s_next] in env.step(s, a)]) for a in range(len(env.actions))])
...
if __name__ == "__main__":
    ...
    for t in range(args.steps):
        value_function = policy_evaluation(policy, value_function, GridWorld, args)
        policy, stable = policy_improvement(policy, value_function, GridWorld, args)
        # if stable: break
