...
def evaluate(rewards):
    ...
    print(simple_moving_average(rewards, 5000)[-n:])
    print("Exponential moving average")
    print(exponential_moving_average(rewards, 0.001)[-n:])
    print("Average")
    print(np.sum(rewards[-n:]) / n)

    # to use this plot function you have to install matplotlib
    # use conda install matplotlib
    plot_series(simple_moving_average(rewards, 5000), "moving_average.png")
